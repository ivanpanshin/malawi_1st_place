...
from reg_malawi.callbacks.base import Callback
from reg_malawi.metrics.base import BaseMetric
...
class Trainer:
    ...
    def val_epoch(self, loader_name: str) -> Dict[str, Any]:
        ...
        for batch_index, (input, labels) in enumerate(tqdm(self.loaders[loader_name])):
            input, labels = input.to(self.device), labels.to(self.device)
        ...
        _metrics = self.metric.calculate()
        self.metric.cleanup()
    ...
    def fit(
        ...
        for epoch in range(self.start_epoch, self.cfg.trainer.trainer_hyps.num_epochs):
            ...
            #self.loaders["train"].dataset.set_epoch(epoch)
            ...
            if self.device == 'cuda:0':
                ...
                for name in val_loaders_names:
                    for k, v in self.val_epoch(loader_name=name).items():
                        self.val_metrics[f'{name}/{k}'] = v

                if self.cfg.logging.track_train_metrics:
                    self.train_metrics = self.val_epoch(loader_name="train")
    ...
    def calculate_test_preds(
        ...
        assert hasattr(self.cfg.trainer.trainer_hyps, 'test_dataset_id_attribute'), "In order to calculate test preds, test trainer should have test_dataset_id_attribute"
        assert hasattr(self.cfg.trainer.trainer_hyps, 'test_preds_output_path'), "In order to calculate test preds, test trainer should have test_preds_output_path"

        total_ids, total_preds_other, total_preds_tin, total_preds_thatch = [], [], [], []

        batch_size = self.loaders['test'].batch_size
        for batch_index, input in enumerate(tqdm(self.loaders['test'])):
            ...
            total_ids[start_index:end_index] = getattr(self.loaders['test'].dataset, self.cfg.trainer.trainer_hyps.test_dataset_id_attribute, f"{self.cfg.trainer.trainer_hyps.test_dataset_id_attribute} attribute not found")[start_index:end_index]

            total_preds_other[start_index:end_index] = preds['other_preds'].cpu()
        ...
        save_preds(
            ...
            total_preds_thatch=total_preds_thatch,
