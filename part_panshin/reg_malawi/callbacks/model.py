...
#    @staticmethod
    ...
    def on_epoch_end(self, trainer):
        ...
        if trainer.metric.main_metric_direction == "maximize":
            item = HeapItem(
            ...
            item = HeapItem(
                -1 * trainer.val_metrics[trainer.metric.main_metric],
                f"epoch_{trainer.current_epoch}_{trainer.metric.main_metric.split('/')[-1]}_{value}.pt",
            )
        deleted_item_path = None
        if len(self.top_items) < self.top_k:
            heapq.heappush(self.top_items, item)
        elif trainer.val_metrics[trainer.metric.main_metric] < -1 * self.top_items[0].metric:
...
class ResumeTraining(Callback):
    ...
    def on_init_end(self, trainer):
        ...
        checkpoint_path = mlflow.artifacts.download_artifacts(run_id=self.mlflow_run_id, artifact_path="best.pt")
...
class LoadModelFromDiskMismatchedKeys(Callback):
    def __init__(self, path):
        super().__init__()
        self.path = path
    ...
    def on_init_end(self, trainer):
        ...
        # Find matching keys with matching shapes
        matching_keys = {k: v for k, v in checkpoint_state_dict.items() if k in model_state_dict and v.size() == model_state_dict[k].size()}
        mismatched_keys = [k for k in checkpoint_state_dict if k not in matching_keys]

        # Log the keys that won't be loaded
        logging.info(f"The following model keys won't be loaded: {mismatched_keys}")

        # Load the matched parameters
        model_state_dict.update(matching_keys)
        trainer.model.module.load_state_dict(model_state_dict, strict=True)

        logging.info("Model state dict updated successfully with matching parameters.")
